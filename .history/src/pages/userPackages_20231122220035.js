...
export default function UserPackages() {
  ...
  const getData = async () => {
    const response = await axios.get(process.env.REACT_PORT + "get_packages");
    const response2 = await axios.get( process.env.REACT_PORT + "get_users");
    const response3 = await axios.get(process.env.REACT_PORT + "get_products");
